...
* `rules.yml` - single source of truth for business rules: management fees matching (with ratios and excludes), the 5-year plan objectives (CA and margin lower bounds per year) and the Attio davai types (one-shot vs MRR billing). Read on every request, strict keys (a typo is an error).
